...
public class UserServiceImpl implements UserService {
    ...
    public void registerNewUserAccount(User user) throws UserAlreadyExistException {
        if (userExist(user.getUsername())) {
            throw new UserAlreadyExistException("{error.user.username.existed} " + user.getUsername());
        }
        if (emailExist(user.getEmail())) {
            throw new UserAlreadyExistException("{error.user.username.existed} " + user.getEmail());
        }
